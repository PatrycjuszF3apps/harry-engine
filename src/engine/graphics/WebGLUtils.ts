...
export class WebGLUtils {
    ...
    static loadTexture(gl: WebGL2RenderingContext, url: string): Promise<WebGLTexture> {
        return new Promise((resolve, reject) => {
            ...
            image.onload = () => {
                const texture = gl.createTexture();
                if (!texture) {
                    reject(new Error("Failed to create WebGLTexture object"));
                    return;
                }

                gl.bindTexture(gl.TEXTURE_2D, texture);

                // Ustawienia dla Pixel Art (ostre krawędzie, brak rozmycia)
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);

                // Odwrócenie osi Y (WebGL ma 0,0 w lewym dolnym rogu)
                gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
